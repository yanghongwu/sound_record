	...
	var disRemainTimeOnPlayingMemIntervalId; //通过播放内存音频显示剩余时间的时间控制器
	...
	function loadMP3CompleteCallBackForFlash(){
	...
	function stopRecordingAfterProcess(){
		...
		//var recordTime = RECORD_TIME - parseInt(remainTime.replace("\"",""));  // 防止与后端时间差一秒；
		//if(recordTime < 0) {recordTime = 0;}
		$(".second").text(Recorder.duration());
	...
	function displayRemainTimeOnPlayingMem(totalTime) {
		var remainTime = Recorder.remainTimeOnPlayingMemAudio(totalTime);
		
		var remainTimeFromPage = $('.second').text();
		$('.second').text(remainTime);
		 	 
		if(remainTime <= 0 || (remainTimeFromPage == remainTime && remainTime == 1)){
			clearInterval(disRemainTimeOnPlayingMemIntervalId);
			$('.second').text($('.second').attr("totalTime"));
		}
	}
	...
	function startLazyRecording(){
		$(".voiceRecord").text("正在加载麦克风......");
		clearTimeout(timeoutID);
		try{
			Recorder.startMicRecording();
		}catch(e){
			timeoutID = setTimeout("startLazyRecording()",500);
			return;
		}
		// 处理没有麦克风的情况
		if(eval(isHasError)) {
