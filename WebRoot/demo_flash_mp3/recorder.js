...
function microphone_recorder_events() {
	...
	case "ready":
		...
		Recorder.uploadFieldName = "upload";
	...
	case "recording":
		...
		}, 1000);// 锟斤拷示录锟斤拷时锟斤拷亩锟绞憋拷锟斤拷锟�		break;
	case "recording_stopped":
		...
		$('#duration').text(duration.toFixed(4) + " seconds");
		$('#play_button').show();
		clearInterval(intervalId); // 锟斤拷锟斤拷锟绞韭硷拷锟绞憋拷锟侥讹拷时锟斤拷锟斤拷
		break;
	case "playing":
		var name = arguments[1];
		$('#record_button img').attr('src', 'images/record.png');
	...
	case "save_progress":
		...
		$('#upload_status').css({
			'color' : '#000'
		}).text(name + " progress: " + bytesLoaded + " / " + bytesTotal);
		break;
	}
}
...
(function() {
	window.Recorder = {
		...
		defaultSize : function(width, height) {
			Recorder.resize(Recorder.recorderOriginalWidth,
					Recorder.recorderOriginalHeight);
		},
		...
		configure : function(rate, gain, silenceLevel, silenceTimeout) {
			...
			Recorder.recorder.configure(rate, gain, silenceLevel,
					silenceTimeout);
		},
